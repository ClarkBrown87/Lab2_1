...
public class AuditoriumAgent extends Agent {
    ...
    protected void setup() {
        ...
        addBehaviour(new AuditoriumBehaviour(this, model, server)); // добавляем поведение агента
    }
}
...
class AuditoriumBehaviour extends SimpleBehaviour {
    ...
    public AuditoriumBehaviour(Agent agent, Auditorium model, AID server) {
        ...
        // готов
        var mes = new ACLMessage(); // создаем сообщение
        mes.addReceiver(this.server); // добавляем получателя
        mes.setConversationId("READY_CHILD"); // идентификатор сообщения
        this.myAgent.send(mes); // отправка сообщения
    }
    ...
    @Override
    public void action() {
        var mes = this.myAgent.receive(); // ожидание получения сообщения
        if (mes != null) {
            print("Получено сообщение от " + mes.getSender().getName() + " " + mes.getConversationId());
            var reply = mes.createReply(); // ответ на полученное сообщение
            try {
                switch (mes.getConversationId()) {
                    case "TIMETABLE_AUDITORIUM" -> {
                        ...
                        reply.setContent(obj.toJSONString()); // заполняем содержимое ответа в виде JSON
                        print("Шлем результаты серверу");
                        myAgent.send(reply);
                    }
                    // отправляем доступные аудитории
                    ...
                    case "whatGroup" -> {
                        // получаем группу в аудитории опр дня и опр пары
